...
import { FiFolder, FiPlus, FiTrash2, FiEdit, FiCheck, FiX } from "react-icons/fi";
...
export default function Categories() {
  ...
  const handleSaveRename = (oldName) => {
    if (editingValue.trim() && editingValue.trim() !== oldName) {
      editCategory(oldName, editingValue.trim());
    }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="md:col-span-2 admin-glass-card rounded-2xl p-5">
        <h3 className="text-sm font-bold text-slate-800 border-b border-slate-100 pb-3 mb-4">Active Categories</h3>
          ...
          {categories.map((cat, idx) => (
            ...
            >
              <div className="flex items-center gap-2 flex-grow mr-2 min-w-0">
                <FiFolder className="text-teal-600 shrink-0" />
                {editingIndex === idx ? (
                  <input
                    type="text"
                    value={editingValue}
